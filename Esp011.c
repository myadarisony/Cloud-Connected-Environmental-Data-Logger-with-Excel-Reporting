...
//		return;
	...
	string_lcd("AT+CWJAP");
	delay_ms(1000);
	string_uart("AT+CWJAP=\"SSID\",\"PASSWORD\"\r\n");
	i=0;memset(buff,'\0',200);
...

{
		...
		if(field==1)
		{
			string_uart("GET /update?api_key=XXXXXXXXXXXXXXXX&field1=");
			cmd_lcd(0x01);
			cmd_lcd(0x80);
		...
		{
	string_uart("GET /update?api_key=XXXXXXXXXXXXXXXX&field2=");
		cmd_lcd(0x01);
		...
		{
	string_uart("GET /update?api_key=XXXXXXXXXXXXXXXX&field3=");
				  	  cmd_lcd(0x01);
             cmd_lcd(0x80);
